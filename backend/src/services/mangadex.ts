...
async function searchMangaMangaDex(title: string): Promise<MangaMetadata | null> {
  try {
    ...
    if (bestScore < 0.6) {
      console.log(
        `MangaDex: Best match score ${(bestScore * 100).toFixed(1)}% too low for: ${title}`
      )
      return null
    }
 ...
 */
export async function searchMangaBatch(
  titles: string[]
): Promise<Map<string, MangaMetadata | null>> {
  ...
  for (let i = 0; i < titles.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(titles.length / BATCH_SIZE)} (${batch.length} items)`
    )
...
export async function getMangaByMangaDexId(mangadexId: string): Promise<MangaMetadata | null> {
  try {
    ...
    console.error(
      `MangaDex error for ID "${mangadexId}":`,
